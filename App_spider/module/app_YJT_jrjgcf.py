...
"""
    手机app预警通 的诚信大数据 金融监管处罚的抓取
    
    todo:   1. 通过对信息的筛选，得到更多的数据。
            2. 解决文件保存的乱码问题。(正在解决) 
"""
...
class Yjt:
    def __init__(self):
        self.u = Util()
        self.url = "https://app.finchina.com/finchinaAPP/getOrgFamilyCreaditData_SE.action?selTopRecommended=%E9%87%91%E8%9E%8D%E7%9B%91%E7%AE%A1%E5%A4%84%E7%BD%9A&skip={}"
            #"https://app.finchina.com/finchinaAPP/getBondsDefaultDynamics.action?pageNum={}&pageSize=15"
        self.file = "Scrapyed_jrjgcf.txt"
        self.headers = {
        ...
        for i in detail_datas:
            ...
            if file_url:
                if str.endswith(file_url, "pdf"):
                    if os.path.exists('./Files/jrjgcf/{}.pdf'.format(file_name)):
                        pass
                    else:
                        with open('./Files/jrjgcf/{}.pdf'.format(file_name), "wb+") as f:
                    ...
                    if os.path.exists('./Files/jrjgcf/{}.pdf'.format(file_name)):
                        ...
                        pdfkit.from_url(file_url, './Files/jrjgcf/{}.pdf'.format(file_name), configuration=confg, options=options)
                elif str.endswith(file_url, "doc"):
                    if os.path.exists('./Files/jrjgcf/{}.doc'.format(file_name)):
                        pass
                    else:
                        with open('./Files/jrjgcf/{}.doc'.format(file_name), "wb+") as f:
                            f.write(requests.get(url=file_url).content)
                            print(file_name + " 保存完成！")
            ...
            self.u.insert2mysql(about_people, self.jrjgcf_sql(jrjgcf_data))
