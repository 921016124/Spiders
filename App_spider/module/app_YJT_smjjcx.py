...
class Yjt:
    def __init__(self):
        self.u = Util()
        self.url = "https://app.finchina.com/finchinaAPP/getOrgFamilyCreaditData_SE.action?selTopRecommended=%E7%A7%81%E5%8B%9F%E5%9F%BA%E9%87%91%E8%AF%9A%E4%BF%A1&skip={}"
            #"https://app.finchina.com/finchinaAPP/getBondsDefaultDynamics.action?pageNum={}&pageSize=15"
        self.file = "Scrapyed_smjjcx.txt"
        self.headers = {
        ...
        for i in detail_datas:
            ...
            if file_url:
                if str.endswith(file_url, "pdf"):
                    if os.path.exists('./Files/smjjcx/{}.pdf'.format(file_name)):
                        pass
                    else:
                        with open('./Files/smjjcx/{}.pdf'.format(file_name), "wb+") as f:
                    ...
                    if os.path.exists('./Files/smjjcx/{}.pdf'.format(file_name)):
                        ...
                        pdfkit.from_url(file_url, './Files/smjjcx/{}.pdf'.format(file_name), configuration=confg, options=options)
                elif str.endswith(file_url, "doc"):
                    if os.path.exists('./Files/smjjcx/{}.doc'.format(file_name)):
                        pass
                    else:
                        with open('./Files/smjjcx/{}.doc'.format(file_name), "wb+") as f:
                            f.write(requests.get(url=file_url).content)
                            print(file_name + " 保存完成！")
            ...
            self.u.insert2mysql(about_people, self.smjjcx_sql(smjjcx_data))

    def smjjcx_sql(self, data):
        sql = """insert into 
        yjt_smjjcx(id, pub_date, about_people,handle_people, punish_type, irregularities, 
