...
"""
    数据来源：千里马招标网
    来源地址：http://www.qianlima.com/zbbg/p1
    数据描述：千里马招标网招标变更信息
    目标表中文名：招标变更信息
    目标表英文名：INVT_PUB_BID_MDF_INF
    数据量：0 - 1 （万条）
    作者：mcg
    状态：完成
    记录时间：2019.08.02
    备注：可以查看200页，每页30条，账户存在每日查看上限，初步估计每日没人可查看100条详情
      普通会员不能查看详情，重要信息都被用 * 号代替了。
"""
            ...
            try:
                ...
                sql = """insert into INVT_PUB_BID_MDF_INF(ID, TTL, ZON, STS, INVT_PUB_BID_CNTNT, ISU_TM, DTL_LINK, INPT_DT,)
                      ...
                      % (record_id,
                         ...
                         detail_url[0],
                         crawl_time)
                up_sql = "update qlm_zbbg_url set status = 1  where url = '{}';".format(detail_url[0])
