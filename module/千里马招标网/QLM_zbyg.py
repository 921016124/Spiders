...
"""
    数据来源：千里马招标网
    来源地址：http://www.qianlima.com/zbyg/p1
    数据描述：千里马招标网招标预告信息
    目标表中文名：招标预告信息表
    目标表英文名：INVT_PUB_BID_PRP_INF
    数据量：0 - 1 （万条）
    作者：mcg
    状态：完成 
    记录时间：2019.08.02
...
class Qlm_zbyg:
    def __init__(self):
        self.base_url = "http://www.qianlima.com/zbyg/p{}"
        self.page = 200
        self.util = Util()
        self.headers = {
    ...
    def get_url_mysql(self):
        for i in range(200):
            ...
            for dl in html.xpath("//div[@class=\"sevenday_list\"]/dl"):
                ...
                sql = "insert into qlm_zbyg_url(url,status) values ('%s','0')" % detail_url
    ...
    def get_mess(self):
        ...
        cursor = conn.cursor()
        sql = "select url from qlm_zbyg_url where status=0;"
        cursor.execute(sql)
            ...
            try:
                ...
                sql = """insert into INVT_PUB_BID_PRP_INF(ID,TTL,ZON,STS,INVT_PUB_BID_CNTNT,ISU_TM,DTL_LINK,INPT_DT)
                      ...
                      % (record_id,
                         ...
                         detail_url[0],
                         crawl_time)
                up_sql = "update qlm_zbyg_url set status = 1  where url = '{}';".format(detail_url[0])
