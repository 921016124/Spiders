...
"""
    数据来源：投资界
    来源地址：https://zdb.pedaily.cn/ipo
    数据描述：投资界所有上市事件信息
    目标表中文名：投资界上市事件
    目标表英文名：INV_LST_EVT_SHH_INF
    数据量：1 -2 （万条） 
    作者：mcg
    状态：完成 
    记录时间：2019.08.02
...
class ZdbPedaily:
    def __init__(self):
        page = int(19687 % 24) if int(19687 % 24) != 0 else int(19687 % 24)+1
        self.urls = ["https://zdb.pedaily.cn/ipo/p{}/".format(i) for i in range(1, page)]
        self.util = Util()
        ...
        self.headers = {
            ...
            "Cookie": "__uid=1452122016; __utmz=23980325.1564110676.19.9.utmcsr=baidu|utmccn=(organic)|utmcmd=organic; ARRAffinity=197ae5372184c64aeca47f780a2e053f3a50366e2bda392cd4bfa3b38e39a929; Hm_lvt_25919c38fb62b67cfb40d17ce3348508=1564110676,1564387906,1564455299,1564997145; __utma=23980325.1444638820.1563415171.1564455299.1564997145.24; __utmc=23980325; __utmt=1; __fromtype=0; Hm_lpvt_25919c38fb62b67cfb40d17ce3348508={}; __utmb=23980325.6.10.1564997145",
            "Host": "zdb.pedaily.cn",
    ...
    def get_shareholder(self, id_code, detail_html):
        ...
        if shareholder_info:
            for si in shareholder_info:
                ...
                sql_sharholder = "insert into INV_LST_EVT_SHH_INF(ID,SHH_INF,SHH_TYP,SSCR_CTRB_AMT,INPT_DT) " \
                                 "values('%s', '%s', '%s', '%s','%s')" % (
    ...
    def get_main_people(self, id_code, detail_html):
        ...
        if main_people:
            for p in main_people:
                ...
                sql_main_people = "insert into INV_LST_EVT_MAIN_PSN_INF(ID, MAIN_PPL_NM, MAIN_PPL_POS, INPT_DT) " \
    ...
    def get_detail_info(self, detail_url):
        ...
        print("==>" + str(detail_res.status_code))
        if detail_res.status_code == 200:
            ...
            if detail_html.xpath("//*[@id=\"contact\"]"):
                contact = "".join(detail_html.xpath("//*[@id=\"contact\"]/p//text()"))
            else:
                contact = ""
            ...
            for rz_html in detail_html.xpath("//div[@class=\"list-invest\"]/ul/li"):
                ...
                if rz_res.status_code == 200:
                    ...
                    sql_sssj = """insert into INV_LST_EVT_BAS_INF(
                                                        ID,CMP_NM,ORG_TOT_DEPT,REG_PLC_PNT,CMP_SET_UP_TM,AFL_IDT,FORMAL_WEB,CMP_INTRO,LVRG_TTL,LVRG_INF,LVRG_INTRO,LGP_RPRS,SET_UP_TM,REG_CPT,OPR_RIT,REG_NBR,APRV_TM,ORG_ORG_CD_NBR,SOC_CRD_CD,TAX_PSN_RCG_NBR,REG_INSTT,ENTP_TYP,INPT_DT,CTC_MTH
                                                        ...
                                                        """ % (id_code,
                                                               ...
                                                               crawl_time,
                                                               contact)
                    self.util.insert2mysql("融资公司信息", sql_sssj)
    ...
    def get_items_list(self, res):
        ...
        for li in html.xpath("//ul[@id=\"ipo-list\"]/li"):
            time.sleep(2)
            # 详情页获取
